...
def title():
	print(''' Please choose from the list_moves above:
                1.Timer
                2.Alarm-Clock
                3.Clockf ''')
...
def user_timer():
	print('Insert  a time. Like(09:00) and to quit press "q" or "quit"')
	...
	if alarm == 'q' or alarm == 'quit':
		...
		if 0 <= int(alarm_list[0]) < 24 and 0 <= int(alarm_list[1]) <= 59 and 0 <= int(alarm_list[2]) <= 60:
			...
			print('Please insert correct hours and minutes! The hours should be less than 24 and minutes less than 60')
	else:
...
def clock_alarm():
	try:
		...
		time_tuple = (
			current_time.tm_year, current_time.tm_mon, current_time.tm_mday, int(alarm_set[0]), int(alarm_set[1]),
			int(alarm_set[2]), current_time.tm_wday, current_time.tm_yday, current_time.tm_isdst)
